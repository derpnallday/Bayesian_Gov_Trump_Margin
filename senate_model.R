...
genMCMC = function( data , sName="s" , yName="y" ,  
  ...
  model {
    ...
    omega ~ dbeta( 50 , 63  ) # prior based on DEM/REP senate ratio
    # omega ~ dbeta( 256 , 276 ) # Skeptical prior for ESP
    kappa <- kappaMinusTwo + 2
    # kappaMinusTwo ~ dgamma( 0.01 , 0.01 )  # mean=1 , sd=10 (generic vague)
    kappaMinusTwo ~ dgamma( 1.105125 , 0.1051249 )  # mode=1 , sd=10m arbitrary small value for K since parties so polarized 
      ...
      for ( t2Idx in 1:Nidx ) {
        ...
        if ( t1Idx > t2Idx) {  
          ...
          plot ( mcmcMat[ptIdx,parName2] , mcmcMat[ptIdx,parName1] , cex.lab=1.75 ,
                 xlab="Democrat" , ylab="Republican" , col="skyblue" )
